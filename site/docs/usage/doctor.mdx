...
Doctor runs verify tasks with the same execution context as `install:` — the active environment, raid vars, and task options all apply, and tasks without an explicit `path:` default to the repo directory for repo-level entries (home for profile-level ones), so a relative check like `test -f package.json` behaves the same no matter where you invoke `raid doctor` from. A failure on one verify entry does not prevent subsequent entries from running, so you see the full health picture in a single pass. Because verify blocks can mutate state through `onFail:` remediation, doctor holds the same cross-process mutation lock as other raid commands while it runs.
