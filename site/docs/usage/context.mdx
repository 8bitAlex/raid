...
Each tool calls straight into the existing raid library and returns its result as a JSON-structured tool result. Mutating tools (`raid_install`, `raid_env_switch`, `raid_run_task`) are serialised behind a single mutex so concurrent calls are processed in order — running two installs against the same workspace at once is rarely what an agent intends.
