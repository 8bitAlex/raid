...
| Tool | Purpose |
|---|---|
| `raid_list_profiles` | List configured profiles |
| `raid_list_repos` | List repos in the active or specified profile |
| `raid_describe_repo` | Return parsed `raid.yaml` as structured JSON |
| `raid_install` | Clone repositories and run install tasks |
| `raid_env_switch` | Switch environments |
| `raid_run_task` | Run a user-defined `raid <command>` |

:::warning Tool handlers are stubs in this initial release
Every tool is registered with its full input schema so MCP hosts can discover the surface area, but `tools/call` currently returns an `isError` result of *"not yet implemented"*. The handlers are landing in follow-up work — track [issue #45](https://github.com/8bitalex/raid/issues/45) for progress.
:::
