...
**Env var.** Set `RAID_HEADLESS=1` (or `true` / `yes` / `on`, case-insensitive) for the same effect as the flag. The flag and env var are interchangeable; the flag works by setting the env var so a single read site in raid serves both.
