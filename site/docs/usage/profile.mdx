...
Fetched profiles are copied to `~/<name>.raid.yaml`. If a file already exists at that path, `raid profile add <url>` refuses with a `PROFILE_ALREADY_EXISTS` error instead of overwriting it — remove or rename the existing file, or register it directly with `raid profile add ~/<name>.raid.yaml`.
