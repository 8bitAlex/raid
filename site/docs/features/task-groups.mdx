...
Groups can reference other groups, nesting to any depth. Reference cycles (a group that — directly or through intermediate groups — references itself) are detected at execution time and reported as a `TASK_FAILED` error naming the cycle chain, rather than recursing forever.
